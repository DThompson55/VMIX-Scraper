...
async function connect(callback){//{httpResponse, status}
        ...
        try {
            ...
            parser.parseString(response.data, (err,result) => {
                callback(err, {vMixCfg:result, vMixStatus:"Connected"})
            })
        } catch (err) {
            if ( err.code ) 
                callback(err, {vMixCfg:null, vMixStatus:err.code+" http://"+err.address+":"+err.port})
            else 
                callback(err, {vMixCfg:null, vMixStatus:"vMix Not Connected?"})
            }
        }
